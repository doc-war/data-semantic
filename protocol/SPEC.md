...
data-semantic的核心是基于html原生的`data-*`规范实现数据的语义和槽位声明。在LLM时代，AI更善于语义建模，先确定IR数据结构，UI侧声明语义槽位，数据侧提供对应字段，运行时 / runtime/编译器完成确定性翻译。

对照示例来解释流程和基础概念。

#### 1、领域IR建模

我们要显示一个极简的页面，只需要显示标题和章节元信息。
...
schema确定，则数据的语义和寻址关系也随之确定。

#### 2、UI声明

UI侧负责消费最终数据，按schema进行槽位声明
...
此示例中的`data-semantic`、`data-semantic-list`、`data-semantic-href`，都是合法的html自定义属性，代表不同的声明语义，即决定要渲染到哪里，怎么渲染。

##### Key

此示例中的属性值`page.title`、`page.chapters`、`.url`、`.name`，决定要渲染什么内容，什么语义，怎么寻址：

* 其值本身，代表一个语义槽位，和未来实际渲染（或编译）的数据形成语义化对照
* 对于UI侧，代表一个需要渲染的数据槽位，表示要按协议以某种方式、某个位置渲染一份数据，除了list语义要求渲染数组外，其余槽位要渲染的都是标量值。
* 对于数据侧，代表一个索引槽位，对应于json中的`key路径`，用于定位到要渲染的数据值。因此该值最终将用于数据求值阶段的寻址解析。

#### 3、数据准备

另一方面，基于IR数据模型，准备需要渲染的实例数据。
...
运行时（Runtime）和编译器（Compiler）负责结合UI声明和实例数据，进行协议运算，完成dom翻译。实际的翻译范围，取决于传入的数据及寻址命中量。
